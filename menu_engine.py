...
def generar_menu_semanal(
    ...
    # ── Ajuste por suplemento proteico ───────────────────────────────────────
    # Si el paciente usa suplemento proteico, descontar esos gramos del objetivo
    # que deben cubrir las comidas. Las comidas se rankean y seleccionan contra
    # la proteína neta (total - suplementaria), no contra el total.
    PROTEINA_SUPLEMENTO_G = 25  # gramos fijos del suplemento proteico
    suplementos = datos.get("suplementos", [])
    usa_suplemento_proteico = "proteina" in [str(s).lower().strip() for s in suplementos]
    proteina_suplementaria = PROTEINA_SUPLEMENTO_G if usa_suplemento_proteico else 0
    proteinas_total = proteinas                          # objetivo total del paciente
    proteinas_comidas = max(proteinas - proteina_suplementaria, 1)  # lo que cubren D+A+C

    if usa_suplemento_proteico:
        print(f"[menu_engine v3.7] Suplemento proteico: {proteina_suplementaria}g → "
              f"objetivo comidas = {proteinas_comidas}g (total = {proteinas_total}g)")

    # objetivos y metas_por_comida usan proteinas_comidas para scoring/selección
    objetivos = {"kcal": kcal, "proteinas": proteinas_comidas, "carbos": carbos, "grasas": grasas}
    ...
    def _construir_combos_busqueda(tam_objetivo: int) -> list:
        ...
        La capa 1 garantiza al menos 1 combo por cada almuerzo_norm y cena_norm
        antes de rellenar con mejores globales. Esto asegura que el beam tenga
        acceso a todos los platos A/C independientemente del score nutricional,
        lo cual es crítico cuando el objetivo proteico cambia (ej. con suplemento).
        """
        vistas: set = set()
        cb: list = []
        ...
        # Capa 1a: mejor combo por cada almuerzo_norm (cobertura garantizada)
        for lst in por_a.values():
            _add(lst[0])  # mejor por score para ese almuerzo

        # Capa 1b: mejor combo por cada cena_norm (cobertura garantizada)
        for lst in por_c.values():
            _add(lst[0])  # mejor por score para esa cena

        # Capa 1c: mejor combo por par (almuerzo_norm, cena_norm)
        for item in por_ac.values():
            _add(item)

        # Capa 2: mejores globales — calidad nutricional
        for item in pool_sorted[:max(200, tam_objetivo // 5)]:
            _add(item)

        # Capa 3: top-N combos por almuerzo_norm — más cobertura cruzada
        top_por_a = max(5, tam_objetivo // 100)
        for lst in por_a.values():
            for item in lst[:top_por_a]:
                _add(item)

        # Capa 4: top-N combos por cena_norm
        ...
        for dia_nombre in dias_semana:
            ...
            for estado in beam:
                ...
                for d, a, c, score_nutri, estado_nutri, d_n, a_n, c_n in combos_pre:
                    # Reglas duras (filtro estricto — sin excepciones)
                    ...
                    # R6 (max 2 desayunos iguales) se aplica como penalización
                    # en _score_ajustado, no como regla dura, para no bloquear
                    # el beam en perfiles con objetivo proteico alto.
            ...
            # Pre-recortar a beam_width * 15 antes de las dos llamadas a heapq
            # para evitar operar sobre listas de 100k+ elementos.
            # combos_pre ya viene ordenado por score, así que los más prometedores
            # por calidad nutricional están al inicio de nuevos_estados (por estado).
            cap = beam_width * 15
            if len(nuevos_estados) > cap:
                nuevos_estados = heapq.nsmallest(cap, nuevos_estados, key=_key_score)
    ...
    # Cuando hay suplemento proteico, el objetivo proteico cambia y el landscape
    # de scores es diferente. El intento 1 estándar puede no alcanzar; usamos
    # parámetros ligeramente más amplios como punto de partida.
    if usa_suplemento_proteico:
        intentos = [
            (1200, 150),   # intento 1 ajustado — cubre perfiles con suplemento
            (2000, 250),   # intento 2
            (2500, 300),   # intento 3 — último recurso
        ]
    else:
        intentos = [
            (800,  120),   # intento 1 — rápido, cubre la mayoría de perfiles
            (1500, 200),   # intento 2 — refuerzo
            (2500, 300),   # intento 3 — último recurso
        ]
    ...
    resumen = {
        ...
        "proteinas_total_objetivo": proteinas_total,
        "proteina_suplementaria": proteina_suplementaria,
        "proteinas_objetivo_comidas": proteinas_comidas,
        "distribucion": {
    ...
    # Las restricciones clínicas (ej. sin vacuno) vienen del campo "restricciones"
    # del paciente, no como reglas fijas del engine.
    ...
    def _generar_opciones_categoria(platos_validos, meta_comida, max_opciones=7):
        ...
        # Agrupar por nombre normalizado — mejor versión de cada nombre
        ...
        for plato in platos_validos:
            ...
            if nombre_norm not in mejores_por_nombre:
                ...
                if calcular_score_individual(plato, meta_comida) < \
                   calcular_score_individual(mejores_por_nombre[nombre_norm], meta_comida):
                    mejores_por_nombre[nombre_norm] = plato
    ...
    # Advertir si alguna categoría quedó con menos de 7 por restricciones del paciente
    for tipo, lista in [("desayunos", opciones_desayunos),
        ...
        if len(lista) < 7:
            resumen["advertencias"].append(
                f"opciones.{tipo}: solo {len(lista)} platos únicos disponibles "
                f"(restricciones aplicadas)"
            )

    print(f"[menu_engine v3.7] Opciones desayunos = {len(opciones_desayunos)}")
    print(f"[menu_engine v3.7] Opciones almuerzos = {len(opciones_almuerzos)}")
    print(f"[menu_engine v3.7] Opciones cenas = {len(opciones_cenas)}")
