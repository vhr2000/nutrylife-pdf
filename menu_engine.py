...
def generar_menu_semanal(
    ...
    # Palabras clave que identifican carne roja / vacuno en nombre o ingredientes.
    # Se usan para filtrar las opciones de cena (no el menú semanal).
    _VACUNO_KEYWORDS = {
        "vacuno", "carne roja", "carne asada", "carne mechada",
        "lomo", "filete", "malaya", "osobuco", "posta", "asiento",
        "hamburguesa de vacuno", "plateada",
    }

    def _es_cena_con_vacuno(plato: Dict) -> bool:
        """Retorna True si el plato de cena contiene carne roja/vacuno."""
        texto = normalizar_texto(
            plato.get("nombre", "") + " " + plato.get("ingredientes", "") + " " +
            " ".join(plato.get("restricciones", []) if isinstance(plato.get("restricciones"), list) else [])
        )
        return any(normalizar_texto(kw) in texto for kw in _VACUNO_KEYWORDS)

    def _generar_opciones_categoria(platos_validos, meta_comida, max_opciones=7,
                                    excluir_vacuno=False):
        """Devuelve hasta max_opciones platos, 1 por nombre normalizado, por score.

        Si excluir_vacuno=True (solo para cenas), intenta primero llenar con platos
        sin carne roja. Si no hay suficientes, completa con los que sí tienen vacuno.
        """
        ...
        # Rankear por score individual
        ordenados = sorted(mejores_por_nombre.values(),
                           key=lambda p: calcular_score_individual(p, meta_comida))

        if not excluir_vacuno:
            return [_formato_plato_salida(p) for p in ordenados[:max_opciones]]

        # Para cenas: separar sin-vacuno y con-vacuno, priorizar sin-vacuno
        sin_vacuno = [p for p in ordenados if not _es_cena_con_vacuno(p)]
        con_vacuno = [p for p in ordenados if     _es_cena_con_vacuno(p)]

        seleccionados = sin_vacuno[:max_opciones]
        if len(seleccionados) < max_opciones:
            # Completar con vacuno si no hay suficientes alternativas
            seleccionados += con_vacuno[:max_opciones - len(seleccionados)]

        return [_formato_plato_salida(p) for p in seleccionados[:max_opciones]]

    opciones_desayunos = _generar_opciones_categoria(
        desayunos, metas_por_comida["desayuno"])
    opciones_almuerzos = _generar_opciones_categoria(
        almuerzos, metas_por_comida["almuerzo"])
    opciones_cenas     = _generar_opciones_categoria(
        cenas, metas_por_comida["cena"], excluir_vacuno=True)

    # Advertir si alguna categoría quedó con menos de 7
    ...
    # Advertir si las cenas tuvieron que incluir vacuno por falta de alternativas
    cenas_con_vacuno_en_opciones = [
        p["nombre"] for p in opciones_cenas if _es_cena_con_vacuno(
            # reconstruir plato desde salida para verificar
            {"nombre": p["nombre"], "ingredientes": p.get("ingredientes", ""),
             "restricciones": p.get("restricciones", "")}
        )
    ]
    cenas_sin_vacuno_ok = len(cenas_con_vacuno_en_opciones) == 0
    if not cenas_sin_vacuno_ok:
        resumen["advertencias"].append(
            f"opciones.cenas: se incluyeron {len(cenas_con_vacuno_en_opciones)} plato(s) "
            f"con vacuno por falta de alternativas suficientes"
        )
    ...
    print(f"[menu_engine v3.7] Opciones cenas sin vacuno = {'Sí' if cenas_sin_vacuno_ok else 'No'}")
