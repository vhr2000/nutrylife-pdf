...
def generar_combinaciones_rankeadas(
    ...
    print(f"[menu_engine v3.6] Candidatos D/A/C = {len(desayunos_cand)}/{len(almuerzos_cand)}/{len(cenas_cand)}")
    print(f"[menu_engine v3.6] Combinaciones evaluadas = {len(combos_válidas)}")
    print(f"[menu_engine v3.6] Nombres únicos en pool D/A/C = {n_d}/{n_a}/{n_c}")
...
def generar_menu_semanal(
    ...
    # ── Selección semanal v3.6: BEAM SEARCH CON REINTENTOS ESCALONADOS ──────
    #
    ...
    def _key_score(estado):
        # Ordenación primaria: menor score → más diversidad A/C → orden estable
        return (estado["score_total"], -len(estado["conteo_ac"]), estado["ids_key"])

    def _key_diversidad(estado):
        # Ordenación alternativa: más diversidad primero, luego mejor score
        # Penaliza concentración (max conteo de un mismo plato A/C)
        max_conc = max(estado["conteo_ac"].values()) if estado["conteo_ac"] else 0
        return (-len(estado["conteo_ac"]), max_conc, estado["score_total"], estado["ids_key"])
    ...
    print(f"[menu_engine v3.6] Candidatos A/C únicos en pool = {len(por_a)}/{len(por_c)}")
    estados_completos = []
    ...
    for n_intento, (n_combos, bw) in enumerate(intentos, start=1):
        ...
        print(f"[menu_engine v3.6] Intento beam {n_intento}: combos={n_combos_real}, width={bw}, éxito={exito}")
        if exito:
            break

    print(f"[menu_engine v3.6] Combos búsqueda semanal = {len(combos_busqueda_pre)}")
    print(f"[menu_engine v3.6] Beam width = {BEAM_WIDTH_USADO}")
    ...
    if estados_completos:
        ...
        print(f"[menu_engine v3.6] Beam completó semana con reglas duras = Sí")
    else:
        ...
        print(f"[menu_engine v3.6] Beam completó semana con reglas duras = No (fallback activado)")
    ...
    # Logs finales
    print(f"[menu_engine v3.6] Conteo desayunos = {dict(sorted(conteo_desayunos_semana.items(), key=lambda x: -x[1]))}")
    print(f"[menu_engine v3.6] Conteo platos A/C = {dict(sorted(conteo_platos_semana.items(), key=lambda x: -x[1]))}")
    ...
    print(f"[menu_engine v3.6] Opciones D/A/C = {len(opciones_desayunos)}/{len(opciones_almuerzos)}/{len(opciones_cenas)}")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("MENU ENGINE v3.6 - OPTIMIZADO PARA RENDER + REGLAS SEMANALES")
    print("=" * 80)
