...
def generar_combinaciones_rankeadas(
    ...
    print(f"[menu_engine v3.5] Candidatos D/A/C = {len(desayunos_cand)}/{len(almuerzos_cand)}/{len(cenas_cand)}")
    print(f"[menu_engine v3.5] Combinaciones evaluadas = {len(combos_válidas)}")
    print(f"[menu_engine v3.5] Nombres únicos en pool D/A/C = {n_d}/{n_a}/{n_c}")
...
def generar_menu_semanal(
    ...
    # ── Selección semanal v3.5: BEAM SEARCH CON REINTENTOS ESCALONADOS ──────
    #
    ...
    print(f"[menu_engine v3.5] Candidatos A/C únicos en pool = {len(por_a)}/{len(por_c)}")
    estados_completos = []
    ...
    for n_intento, (n_combos, bw) in enumerate(intentos, start=1):
        ...
        print(f"[menu_engine v3.5] Intento beam {n_intento}: combos={n_combos_real}, width={bw}, éxito={exito}")
        if exito:
            break

    print(f"[menu_engine v3.5] Combos búsqueda semanal = {len(combos_busqueda_pre)}")
    print(f"[menu_engine v3.5] Beam width = {BEAM_WIDTH_USADO}")
    ...
    if estados_completos:
        ...
        print(f"[menu_engine v3.5] Beam completó semana con reglas duras = Sí")
    else:
        ...
        print(f"[menu_engine v3.5] Beam completó semana con reglas duras = No (fallback activado)")
    ...
    # Log conteo final de platos A/C
    print(f"[menu_engine v3.5] Conteo platos A/C = {dict(sorted(conteo_platos_semana.items(), key=lambda x: -x[1]))}")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("MENU ENGINE v3.5 - OPTIMIZADO PARA RENDER + REGLAS SEMANALES")
    print("=" * 80)
