...
SUPLEMENTOS_INFO = {
    ...
    "proteina":    {"nombre": "Suplemento proteico",   "dosis": "25 g",     "momento": "1 vez al día",                    "objetivo": "Apoyo para alcanzar el objetivo diario de proteína y preservar masa muscular"},
}
...
def resolve_image_path(img_ref):
    ...
    # PASO 0: buscar candidatos exactos en todas las bases.
    # Prioridad: campo "imagen" del plato (ej: "D004.jpg") → luego id (ej: "D004-F" → "D004").
    # Para dicts: construir lista de stems a probar en orden.
    if isinstance(img_ref, dict):
        _p0_stems = []
        # 1º campo imagen/foto (nombre de archivo directo, sin sufijo de versión)
        for _k in ["imagen", "image", "foto", "photo", "filename"]:
            _v = img_ref.get(_k)
            if _v:
                _p0_stems.append(os.path.splitext(str(_v))[0])
                break
        # 2º id sin sufijo de versión
        if exact_id:
            import re as _re_p0
            _m = _re_p0.match(r'^([A-Za-z0-9]+)-[A-Za-z]$', exact_id)
            _p0_stems.append(_m.group(1) if _m else exact_id)
        # eliminar duplicados preservando orden
        _p0_seen = []
        for _s in _p0_stems:
            if _s and _s not in _p0_seen:
                _p0_seen.append(_s)
        _p0_stems = _p0_seen
    else:
        # String: probar tal cual y sin sufijo
        import re as _re_p0
        _m = _re_p0.match(r'^([A-Za-z0-9]+)-[A-Za-z]$', exact_id) if exact_id else None
        _p0_stems = list({exact_id, _m.group(1) if _m else exact_id} - {""}) if exact_id else []

    if _p0_stems:
        if DEBUG_IMAGES:
            print(f"🔍 Buscando {_p0_stems} en bases de imágenes...")
        for base in image_search_bases():
            if not os.path.isdir(base):
                continue
            try:
                archivos_lower = {f.lower(): f for f in os.listdir(base)}
                for stem_try in _p0_stems:
                    for ext in [".jpg", ".jpeg", ".png", ".webp"]:
                        fname_try = (stem_try + ext).lower()
                        if fname_try in archivos_lower:
                            found = os.path.join(base, archivos_lower[fname_try])
                            if DEBUG_IMAGES:
        ...
        if not result and DEBUG_IMAGES:
            print(f"❌ {_p0_stems} no encontrado en ninguna base")
...
def add_image_safe(slide, img_ref, left, top, width, height):
    ...
    Log mínimo siempre: una línea por imagen (encontrada o no).
    Rutas detalladas solo con DEBUG_IMAGES=1.
    """
    path = resolve_image_path(img_ref)
    if isinstance(img_ref, dict):
        label = img_ref.get("imagen") or img_ref.get("id") or "?"
    else:
        label = str(img_ref)
    ...
    print(f"🖼️  [{label}] no encontrada → placeholder")
    if DEBUG_IMAGES:
...
def find_image(*names):
    ...
    Hace búsqueda directa en disco — no usa _IMAGE_RESOLVE_CACHE — porque estas
    imágenes pueden no existir en el momento de construir el índice pero sí al
    momento de insertar.
    Emite un log mínimo siempre (encontrada o no) para facilitar diagnóstico.
    """
    ...
    _bases_seen = []
    for _b in _bases:
        if _b and _b not in _bases_seen:
            _bases_seen.append(_b)
    _bases = _bases_seen
    ...
    for name in names:
        ...
        for base in _bases:
            ...
            try:
                ...
                continue
            for ext in _EXTS:
                fname_try = (stem + ext).lower()
                if fname_try in archivos_lower:
                    found = os.path.join(base, archivos_lower[fname_try])
                    print(f"🖼️  [{name}] encontrada: {found}")
                    return _maybe_convert_image_for_pptx(found)

    print(f"🖼️  [{', '.join(names)}] no encontrada en imagenes/")
    return None
...
def generar_pptx_completo(datos, output_path, excel_path=None):
    ...
    menu_resultado = {}  # inicializar aquí — disponible aunque el engine falle
    ...
    # Imágenes del ritmo del día: usar los platos reales seleccionados por el engine.
    # Fallback a códigos genéricos si alguna lista viene vacía.
    _ritmo_platos = [
        desayunos_sel[0] if desayunos_sel else {"id": "D001"},
        almuerzos_sel[0] if almuerzos_sel else {"id": "A001"},
        cenas_sel[0]     if cenas_sel     else {"id": "C001"},
    ]
    horarios = [
    ...
    for i, (hora, comida, desc) in enumerate(horarios):
        ...
        add_image_safe(s, _ritmo_platos[i], img_x, y, RITMO_IMG_W, RITMO_IMG_H)
        if i < 2:
    ...
    # ════════════════════════════════════════════════════════
    # SLIDES — DESAYUNOS / ALMUERZOS / CENAS PARA TI (3 slides separados)
    # ════════════════════════════════════════════════════════

    # Fuente de datos para opciones:
    # - Si menu_engine devolvió menu_resultado["opciones"], usar esas listas
    #   (ya deduplicas por nombre desde el engine).
    # - Fallback: deduplicar desde las listas de la semana.
    _opciones = menu_resultado.get("opciones", {})

    _desayunos_cat = (
        _opciones.get("desayunos")
        or deduplicar_platos_por_nombre(desayunos_sel)
    )
    _almuerzos_cat = (
        _opciones.get("almuerzos")
        or deduplicar_platos_por_nombre(almuerzos_sel)
    )
    _cenas_cat = (
        _opciones.get("cenas")
        or deduplicar_platos_por_nombre(cenas_sel)
    )

    for titulo, platos, color_acento, sec_num in [
        (f"Desayunos para {nombre1}",  _desayunos_cat, PISTACHO, 8),
        (f"Almuerzos para {nombre1}",  _almuerzos_cat, PISTACHO, 9),
        (f"Cenas para {nombre1}",      _cenas_cat,     LILA,     10),
    ]:
        s = new_slide(prs)
        section_header(s, sec_num, titulo, "opciones seleccionadas para tu semana")
