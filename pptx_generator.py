...
def generar_pptx_completo(datos, output_path):
    ...
    def asegurar_7(aptos, originales):
        """Garantiza siempre 7 platos. Si hay menos, rota los aptos o completa con los menos
        restrictivos del catálogo original para no romper el menú semanal."""
        if len(aptos) >= 7:
            return aptos[:7]
    ...
    almuerzos_filtrados = filtrar_por_restricciones(ALMUERZOS, rest_lista)
    cenas_filtradas     = filtrar_por_restricciones(CENAS, rest_lista)

    desayunos_sel = asegurar_7(desayunos_filtrados, DESAYUNOS)
    almuerzos_sel = asegurar_7(almuerzos_filtrados, ALMUERZOS)
    cenas_sel     = asegurar_7(cenas_filtradas, CENAS)

    for nombre_cat, filtrados, sel in [
        ("Desayunos", desayunos_filtrados, desayunos_sel),
        ("Almuerzos", almuerzos_filtrados, almuerzos_sel),
        ("Cenas",     cenas_filtradas,     cenas_sel),
    ]:
        if len(filtrados) < 7:
            print(f"⚠️ {nombre_cat}: solo {len(filtrados)} platos aptos, se completó con rotación para cubrir los 7 días.")
