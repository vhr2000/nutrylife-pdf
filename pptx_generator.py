...
def find_image(*names):
    """Busca imágenes habituales con coincidencia exacta y también parcial.

    Esto ayuda especialmente con portada/myriam cuando el archivo viene como
    'foto_portada_2026.jpg', 'portada plan.jpg', 'Myriam foto.png', etc.
    """
    # 1) búsqueda exacta/normalizada estándar
    for name in names:
    ...
    # 2) fallback: coincidencia parcial sobre el índice cacheado
    idx = image_index()
    wanted = [_norm_image_name(n) for n in names if n]
    for key, path in idx.items():
        if any(w and (w in key or key in w) for w in wanted):
            print(f"✅ Imagen encontrada por coincidencia parcial: {names} -> {path}")
            return _maybe_convert_image_for_pptx(path)
...
def generar_pptx_completo(datos, output_path):
    ...
    for i, (num, titulo) in enumerate(secciones):
        ...
        add_textbox(s, num, x_num, y, inches(0.42), Pt(13),
                    font_name=SERIF, font_size=15.0, color=PISTACHO)
        add_textbox(s, titulo, x_txt, y, W - inches(2.25), Pt(15),
                    font_name=SANS, font_size=13.5, color=OSCURO)
    ...
    add_textbox(s, "Si hay días que solo puedes hacer dos comidas,\nestá bien — escucha tu hambre y respétala.",
                inches(0.95), inches(7.55), W - inches(1.90), Pt(60),
