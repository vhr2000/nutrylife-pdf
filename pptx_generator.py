...
def image_id_candidates(img_ref):
    ...
    for raw in refs:
        ...
        # Variantes C001 directas — NO generar candidatos AC para evitar confusión con almuerzos
        if up.startswith("C") and up[1:].isdigit():
            n = int(up[1:])
            add(f"C{n:03d}"); add(f"C{n:02d}"); add(f"C{n}")
            add(f"Cena{n:03d}"); add(f"Cena{n}")
            # También buscar como AC0(10+n) por si las cenas están en el mismo archivo que almuerzos
            add(f"AC{n+10:03d}"); add(f"AC{n+10:02d}")
...
def generar_pptx_completo(datos, output_path):
    ...
    # Layout bienvenida: 2 columnas claras sin traslape
    # Col derecha: foto de Mimi (ancha) + nombre en 1 línea
    fw, fh = inches(1.80), inches(1.40)
    fx = W - inches(0.65) - fw
    fy = inches(2.60)
    ...
    add_textbox(s, spaced("TU NUTRICIONISTA"), fx, fy + fh + Pt(5), fw, Pt(12),
                font_name=SANS, font_size=5.5, color=GRIS, align=PP_ALIGN.CENTER)
    # Nombre en una sola línea — fuente más pequeña para caber
    add_textbox(s, "Myriam Márquez García", fx - Pt(4), fy + fh + Pt(17), fw + Pt(8), Pt(16),
                font_name=SERIF, font_size=11.5, color=OSCURO, align=PP_ALIGN.CENTER, word_wrap=False)

    # Col izquierda: 3 párrafos en UN solo textbox grande para evitar traslapes
    texto_w = fx - inches(0.65) - inches(0.20)
    texto_completo = (
        "Este plan fue diseñado pensando en ti: en tu cuerpo, tus tiempos, tus gustos "
        "y, sobre todo, en tu objetivo. No es una dieta. Es una propuesta de alimentación "
        "pensada para acompañarte durante las próximas semanas y ayudarte a construir "
        "hábitos sostenibles, con flexibilidad y sin culpa.\n\n"
        "Vas a encontrar acá tres cosas: un marco claro de qué comer y cuánto, un sistema "
        "de opciones e intercambios para que armes tus comidas con libertad, y una serie "
        "de recetas que pueden inspirarte cuando no sepas qué preparar.\n\n"
        "Lo más importante: este plan se ajusta a ti, no al revés. Si algo no calza con "
        "tu rutina, hablémoslo. Estoy aquí para acompañarte."
    )
    add_textbox(s, texto_completo, inches(0.65), inches(2.60), texto_w, inches(5.0),
                font_name=SANS, font_size=12.5, color=OSCURO, word_wrap=True, line_spacing=16)

    add_textbox(s, "Con cariño,", inches(0.65), inches(8.20), inches(3), Pt(20),
                font_size=18, italic=True, font_name=SERIF, color=OSCURO)
    add_textbox(s, "Myriam", inches(0.65), inches(8.50), inches(3), Pt(38),
                font_size=40, italic=True, font_name=SERIF, color=PISTACHO)
    ...
    for idx, dia in enumerate(DIAS):
        ...
        for ci, (emoji, label, plato, bg, acento) in enumerate(comidas):
            y_base = inches(1.65) + ci * inches(3.10)

            # Badge de etiqueta
            add_rect(s, inches(0.5), y_base, inches(2.4), Pt(22), fill_color=bg)
            add_textbox(s, label, inches(0.6), y_base + Pt(4),
                        inches(2.3), Pt(14), font_size=8.5, color=acento, bold=True)
