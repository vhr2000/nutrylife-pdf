...
def resolve_image_path(img_ref):
    ...
    # 4) Segundo intento más flexible: buscar por sufijo numérico solo si coincide el prefijo
    img_ref_str = str(img_ref.get("id", img_ref) if isinstance(img_ref, dict) else img_ref).upper()
    for wn in wanted_norm:
        if wn in index:
