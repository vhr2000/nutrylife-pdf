...
# Cache de resolución: evita recorrer bases para la misma imagen dos veces.
# clave: ref normalizada (str)  →  valor: path resuelto (str) o None
_IMAGE_RESOLVE_CACHE: dict = {}

# Cache de conversión WEBP→JPEG: evita convertir el mismo archivo más de una vez.
# clave: path original (str)  →  valor: path JPEG temporal (str)
_IMAGE_CONVERT_CACHE: dict = {}

# Flag de debug: poner DEBUG_IMAGES=1 en las variables de entorno de Render para
# activar los logs de búsqueda extendidos. En producción mantener en 0.
DEBUG_IMAGES: bool = os.getenv("DEBUG_IMAGES", "0") == "1"
...
def image_search_bases():
    """Rutas base donde buscar imágenes — versión compacta para Render.

    Orden de prioridad:
    1. Variable de entorno IMAGE_DIR / IMAGES_DIR (override explícito).
    2. imagenes/ junto al script  ← ruta principal en producción.
    3. Carpeta del script.
    4. Directorio de trabajo actual.
    5. imagenes/ dentro del directorio de trabajo actual.
    6. /opt/render/project/src  ← raíz del repo en Render.
    7. /opt/render/project/src/imagenes

    La versión anterior recorría ~85 rutas (padres del script con todas sus
    subcarpetas) lo que generaba logs enormes y consumía tiempo en Render.
    """
    ...
    raw = [
        ...
        IMG_BASE,                                      # imagenes/ junto al script
        script_dir,                                    # mismo directorio del script
        cwd,                                           # directorio de trabajo
        os.path.join(cwd, "imagenes"),                 # imagenes/ en cwd
        "/opt/render/project/src",                     # raíz repo en Render
        os.path.join("/opt/render/project/src", "imagenes"),
    ]
...
def image_index():
    ...
    if DEBUG_IMAGES:
        ...
        print(f"📷 Índice construido: {len(set(index.values()))} imágenes encontradas.")
    return index
...
def _maybe_convert_image_for_pptx(path):
    ...
    Cachea el resultado para que la misma imagen no se convierta más de una vez.
    """
    # Consultar cache de conversión antes de abrir PIL
    ...
    try:
        ...
        with Image.open(path) as im:
            ...
            if fmt in ("JPEG", "PNG", "BMP", "GIF", "TIFF"):
                _IMAGE_CONVERT_CACHE[path] = path  # no necesita conversión
                return path
            # Formato no soportado (WEBP, AVIF, etc.) — convertir a JPEG una sola vez
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".jpg")
            tmp.close()
            im.convert("RGB").save(tmp.name, "JPEG", quality=92)
            if DEBUG_IMAGES:
                print(f"🔁 {fmt} convertido a JPEG: {path} -> {tmp.name}")
            _IMAGE_CONVERT_CACHE[path] = tmp.name
            return tmp.name
    except Exception as e:
        if DEBUG_IMAGES:
            print(f"⚠️ No se pudo verificar/convertir {path}: {e}")
        _IMAGE_CONVERT_CACHE[path] = path  # cachear como está para no reintentar
        return path
...
def resolve_image_path(img_ref):
    ...
    Prioridad:
    1. Campo 'imagen' del dict del plato (nombre de archivo real, ej: 'D004.jpg').
    2. ID exacto del plato (ej: 'D004-X' → busca 'D004-X' y también 'D004').
    3. Candidatos alternativos vía image_id_candidates().
    4. Coincidencia normalizada en bases.
    5. Índice cacheado.

    Cachea cada resolución para no repetir la búsqueda por la misma referencia.
    """
    if not img_ref:
        return None

    # Construir clave de cache normalizada
    if isinstance(img_ref, dict):
        cache_key = img_ref.get("imagen") or img_ref.get("id") or str(img_ref)
    else:
        cache_key = str(img_ref)

    # Consultar cache de resolución — evita recorrer bases dos veces para la misma imagen
    if cache_key in _IMAGE_RESOLVE_CACHE:
        return _IMAGE_RESOLVE_CACHE[cache_key]

    exts = [".jpg", ".jpeg", ".png", ".webp", ".JPG", ".JPEG", ".PNG", ".WEBP"]

    def _store_and_return(path):
        """Guarda en cache y retorna el path (puede ser None)."""
        _IMAGE_RESOLVE_CACHE[cache_key] = path
        return path

    # ── PASO 0: usar campo 'imagen' si existe (más directo que el ID) ──────
    if isinstance(img_ref, dict) and img_ref.get("imagen"):
        img_filename = img_ref["imagen"]
        # Buscar el archivo exacto en las bases
        for base in image_search_bases():
            ...
            if os.path.exists(path):
                if DEBUG_IMAGES:
                    print(f"✅ imagen por campo 'imagen': {path}")
                return _store_and_return(_maybe_convert_image_for_pptx(path))
            # También sin extensión + extensiones conocidas
            stem = os.path.splitext(img_filename)[0]
            for ext in exts:
                path = os.path.join(base, stem + ext)
                if os.path.exists(path):
                    if DEBUG_IMAGES:
                        print(f"✅ imagen por campo 'imagen' (ext): {path}")
                    return _store_and_return(_maybe_convert_image_for_pptx(path))

    # ── PASO 1: buscar el ID exacto del plato en las bases ─────────────────
    if isinstance(img_ref, dict):
            ...
            try:
                ...
                for fname in archivos:
                    ...
                    if name_no_ext.lower() == exact_lower and fext.lower() in ['.jpg', '.jpeg', '.png', '.webp']:
                        ...
                        return _store_and_return(_maybe_convert_image_for_pptx(found))
                for ext in exts:
    ...
    # ── PASO 2: candidatos alternativos ────────────────────────────────────
    ...
    # ── PASO 3: coincidencia normalizada en bases principales ──────────────
    for base in image_search_bases():
        if not os.path.isdir(base):
            continue
        try:
            for fname in os.listdir(base):
                if os.path.splitext(fname)[1].lower() in [e.lower() for e in exts]:
                    ...
                    if fn in wanted_norm:
                        return _store_and_return(_maybe_convert_image_for_pptx(os.path.join(base, fname)))
        except Exception:
            continue

    # ── PASO 4: índice cacheado (os.walk acotado) ──────────────────────────
    index = image_index()
    for wn in wanted_norm:
        if wn in index:
            ...
            if found_norm == wn:
                return _store_and_return(_maybe_convert_image_for_pptx(found_path))

    return _store_and_return(None)

def add_image_safe(slide, img_ref, left, top, width, height):
    """Agrega imagen si existe; si no, pone placeholder."""
    ...
    if path:
        try:
            ...
            print(f"⚠️ Imagen encontrada pero no insertable ({label}): {e}")

    # Imagen no encontrada — log mínimo en producción
    print(f"⚠️ Imagen no encontrada: {label}")
    if DEBUG_IMAGES:
        cand = ", ".join(image_id_candidates(img_ref))
        print(f"   Candidatos: {cand}")
...
def diagnosticar_imagenes(platos):
    """Imprime diagnóstico de imágenes. Solo corre si DEBUG_IMAGES=1.

    En producción (DEBUG_IMAGES=0) no hace nada para evitar logs enormes
    y el costo de recorrer carpetas por cada plato.
    """
    if not DEBUG_IMAGES:
        return
