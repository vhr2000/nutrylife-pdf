...
def image_search_bases():
    ...
    # Render: las fotos están en el directorio raíz del repo
    raw += ["/opt/render/project/src"]
...
def resolve_image_path(img_ref):
    ...
    # Buscar con case-insensitive para manejar D005.jpg vs D005.JPG
    if exact_id:
        for base in image_search_bases():
            ...
            # Si no, buscar case-insensitive en el directorio
            try:
                exact_lower = exact_id.lower()
                for fname in os.listdir(base):
                    name_no_ext, fext = os.path.splitext(fname)
                    if name_no_ext.lower() == exact_lower and fext.lower() in [e.lower() for e in exts]:
