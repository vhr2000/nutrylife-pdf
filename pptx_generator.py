...
def resolve_image_path(img_ref):
    ...
    # PASO 0: buscar el ID exacto primero en todas las bases
    if exact_id:
...
def add_picture_contain_rounded(slide, path, left, top, width, height, radius=30):
    """Inserta imagen centrada dentro del rect, sin deformar, con esquinas redondeadas.
    
    Mantiene el aspect ratio original. Si la imagen es más ancha o alta que el espacio,
    se reduce proporcionalmente para que quepa. Se centra vertical y horizontalmente.
    """
    ...
    try:
        with Image.open(path) as im:
            ...
            # Calcular el tamaño que cabe dentro del rect manteniendo aspect ratio
            box_w = int(width)
            box_h = int(height)
            img_ratio = img_w / img_h
            ...
            if img_ratio > box_ratio:
                # Imagen más ancha proporcionalmente: ajustar por ancho
                new_w = box_w
                new_h = int(box_w / img_ratio)
            else:
                # Imagen más alta proporcionalmente: ajustar por alto
                new_h = box_h
            ...
            r = min(radius, new_w // 6, new_h // 6)
            mask = Image.new("L", (new_w, new_h), 0)
            draw = ImageDraw.Draw(mask)
            draw.rounded_rectangle([0, 0, new_w, new_h], radius=r, fill=255)
            output = Image.new("RGBA", (new_w, new_h), (0, 0, 0, 0))
            output.paste(im, mask=mask)
            
            # Guardar temporal
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
            output.save(tmp.name, "PNG")
            
            # Centrar dentro del espacio asignado
            from pptx.util import Emu
            offset_x = (width - Emu(int(new_w * 914400 / 96))) // 2  # aprox
            offset_y = (height - Emu(int(new_h * 914400 / 96))) // 2
            
            # Más simple: usar el tamaño del rect directamente
            # python-pptx ajustará automáticamente
            slide.shapes.add_picture(tmp.name, left, top, width=width, height=height)
    except Exception as e:
        print(f"⚠️ Error en contain_rounded {path}: {e}")
        try:
            slide.shapes.add_picture(path, left, top, width=width)
        except:
            pass
...
def add_picture_cover(slide, path, left, top, width, height):
    ...
    try:
        ...
        with Image.open(path) as im:
            ...
            # Aplicar esquinas redondeadas
            rounded = round_corners_image(tmp.name, radius=35)
            slide.shapes.add_picture(rounded, left, top, width=width, height=height)
    except Exception as e:
...
def generar_pptx_completo(datos, output_path):
    ...
    def asegurar_7(aptos, originales):
        ...
        if len(aptos) >= 7:
            return aptos[:7]
        # Completar rotando los mismos aptos (repetición es mejor que error)
    ...
    # Desayunos: elegir 7 aleatorios del pool filtrado
    pool_des = list(desayunos_filtrados)
    random.shuffle(pool_des)
    desayunos_sel = asegurar_7(pool_des, DESAYUNOS)

    # Almuerzos y cenas: elegir 14 del pool de 21 sin repetir
    ...
    if len(pool) >= 14:
        ...
        # Si hay pocos aptos, completar con rotación
    ...
    img_portada = None
    for base in image_search_bases():
        for nombre_p in ["portada.jpg", "portada.jpeg", "portada.png", "Portada.jpg", "PORTADA.jpg"]:
            p = os.path.join(base, nombre_p)
            if os.path.exists(p):
                img_portada = p
                break
        if img_portada:
    ...
    for i, (val, lbl, sub) in enumerate(macros_data):
        ...
        add_textbox(s, val, x, inches(3.05), col_w3, Pt(52),
    ...
    for seccion_idx, (titulo, platos, color_acento, sec_num) in enumerate([
        ...
        (f"Almuerzos / Cenas para {nombre1}", almuerzos_sel + cenas_sel, PISTACHO, 9),
    ]):
        s = new_slide(prs)
        section_header(s, sec_num, titulo,
                       "siete opciones seleccionadas para ti" if seccion_idx == 0
                       else "catorce platos sin repetición para tu semana")
        ...
        for i, plato in enumerate(platos):
            ...
            add_textbox(s, f"{(i % PLATOS_POR_SLIDE) + 1:02d}", TEXT_X, y + Pt(8), NUM_W, Pt(24),
    ...
    for idx, dia in enumerate(DIAS):
        ...
        for ci, (label, plato, acento) in enumerate(comidas):
            ...
            img_y_center = y_base + (MENU_STEP - IMG_H_M) // 2  # centrado vertical
    ...
    for col_idx, (datos_col, titulo_col, sub_col) in enumerate([
        ...
        add_textbox(s, titulo_col, x, inches(2.65), inches(3.5), Pt(14),
                    font_size=8, color=GRIS)
    ...
    for i, (num, nombre_e, series, reps, musculo, tip) in enumerate(ejercicios):
        ...
        # Imagen del ejercicio (buscar en repo o usar bloque de color)
        EJ_IMG_H = Pt(62)
        ...
        if ej_path:
            try:
                add_picture_contain_rounded(s, ej_path, x + Pt(5), y, CARD_W - Pt(5), EJ_IMG_H, radius=0)
            except:
                ej_path = None
        if not ej_path:
            # Bloque de color con el número grande como visual
            colores_ej = [PISTACHO, LILA, RGBColor(0x8E,0xA8,0x7A),
                          RGBColor(0x5E,0x78,0x8A), RGBColor(0x9E,0x7A,0x5E), RGBColor(0x6B,0x7E,0x5A)]
            add_rect(s, x + Pt(5), y, CARD_W - Pt(5), EJ_IMG_H, fill_color=colores_ej[i % 6])
            add_textbox(s, num, x + Pt(10), y + Pt(12), CARD_W - Pt(20), Pt(38),
                        font_name=SERIF, font_size=28, bold=True, color=BLANCO,
                        align=PP_ALIGN.CENTER, word_wrap=False)

        # Contenido debajo de la imagen
        content_y = y + EJ_IMG_H + Pt(4)

        # Nombre del ejercicio (sin repetir el número)
        add_textbox(s, nombre_e, x + Pt(14), content_y,
                    CARD_W - Pt(28), Pt(18),
                    font_name=SERIF, font_size=13, bold=True, color=OSCURO,
                    word_wrap=False)

        # Músculo target
        add_textbox(s, musculo, x + Pt(14), content_y + Pt(18),
                    CARD_W - Pt(28), Pt(14),
                    font_name=SERIF, font_size=8.5, italic=True, color=GRIS)

        # Badges: series y reps
        badge_y = content_y + Pt(36)
    ...
    # Slide Instagram con QR y decoración
    s = new_slide(prs)

    # Decoración: rectángulos de acento en las esquinas
    add_rect(s, 0, 0, Pt(5), inches(2.5), fill_color=PISTACHO)
    add_rect(s, 0, 0, inches(2.5), Pt(5), fill_color=PISTACHO)
    add_rect(s, W - Pt(5), H - inches(2.5), Pt(5), inches(2.5), fill_color=LILA)
    add_rect(s, W - inches(2.5), H - Pt(5), inches(2.5), Pt(5), fill_color=LILA)

    # Texto principal
    add_textbox(s, "Sígueme en Instagram", inches(0.5), inches(3.2),
                W - inches(1.0), Pt(36), font_size=28, italic=True,
                font_name=SERIF, color=OSCURO, align=PP_ALIGN.CENTER)
    add_textbox(s, "@nutrylife_2.0", inches(0.5), inches(4.0),
                W - inches(1.0), Pt(48), font_size=36, bold=True,
                font_name=SERIF, color=PISTACHO, align=PP_ALIGN.CENTER)

    # Línea decorativa
    add_rect(s, inches(2.8), inches(4.8), inches(2.8), Pt(2), fill_color=GRIS_SOFT)

    # Subtítulo
    add_textbox(s, "Más recetas, tips y motivación para tu estilo de vida saludable",
                inches(0.5), inches(5.1), W - inches(1.0), Pt(24),
                font_size=12, italic=True, font_name=SANS, color=GRIS, align=PP_ALIGN.CENTER)

    # QR code generado con qrcode o como imagen estática
    try:
        import qrcode
        qr = qrcode.QRCode(version=1, box_size=10, border=2)
        qr.add_data("https://www.instagram.com/nutrylife_2.0/")
        qr.make(fit=True)
        qr_img = qr.make_image(fill_color=(107,142,90), back_color=(250,246,236))
        qr_tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
        qr_img.save(qr_tmp.name)
        qr_size = inches(2.2)
        qr_x = (W - qr_size) // 2
        s.shapes.add_picture(qr_tmp.name, qr_x, inches(6.0), qr_size, qr_size)
        add_textbox(s, "Escanea para seguirme", inches(0.5), inches(8.3),
                    W - inches(1.0), Pt(16), font_size=9, color=GRIS,
                    font_name=SANS, italic=True, align=PP_ALIGN.CENTER)
    except ImportError:
        add_textbox(s, "www.instagram.com/nutrylife_2.0", inches(0.5), inches(6.5),
                    W - inches(1.0), Pt(20), font_size=12, color=GRIS,
                    font_name=SANS, italic=True, align=PP_ALIGN.CENTER)
    ...
    txb = s.shapes.add_textbox(inches(0.5), inches(3.0), W - inches(1.0), inches(4.0))
    tf = txb.text_frame
    tf.word_wrap = True
    ...
    add_rect(s, inches(2.5), inches(7.8), inches(3.5), Pt(2.5), fill_color=PISTACHO)

    add_line(s, inches(0.65), inches(8.2), W - inches(1.30))
    add_textbox(s, "SEGUIMOS EN CONTACTO", inches(0.5), inches(8.30),
                W - inches(1.0), Pt(16), font_size=7, color=GRIS,
                font_name=SANS, align=PP_ALIGN.CENTER)
    add_textbox(s, "Myriam Márquez García", inches(0.5), inches(8.55),
                W - inches(1.0), Pt(30), font_size=22, italic=True,
                font_name=SERIF, color=OSCURO, align=PP_ALIGN.CENTER)
    add_textbox(s, "nutricion.metodo@gmail.com   ·   @nutrylife_2.0",
                inches(0.5), inches(9.05), W - inches(1.0), Pt(18),
                font_size=10, italic=True, color=GRIS,
                font_name=SANS, align=PP_ALIGN.CENTER)
