...
def generar_pptx_completo(datos, output_path):
    ...
    def filtrar_por_restricciones(lista, restricciones_raw):
        """Excluye platos cuyo nombre o descripción mencionen ingredientes restringidos."""
        if not restricciones_raw:
            return lista
        # Normalizar: minúsculas, sin tildes básicas
        import unicodedata
        def norm(s):
        ...
        for plato in lista:
            ...
            if not excluir:
                filtrados.append(plato)
        return filtrados

    rest_lista = restricciones if isinstance(restricciones, list) else []
    desayunos_ok = filtrar_por_restricciones(DESAYUNOS, rest_lista)
    almuerzos_ok = filtrar_por_restricciones(ALMUERZOS, rest_lista)
    cenas_ok     = filtrar_por_restricciones(CENAS, rest_lista)

    # Si después del filtro quedan menos de 7, completar con los excluidos al final
    def completar(filtrados, originales, cantidad):
    ...
    desayunos_sel = completar(desayunos_ok, DESAYUNOS, 7)
    almuerzos_sel = completar(almuerzos_ok, ALMUERZOS, 7)
    cenas_sel     = completar(cenas_ok, CENAS, 7)
    ...
    # Diagrama visual: barras verticales proporcionales como plato elegante
    es_carbos = (version == "con_carbos")
    segmentos = [
        (PISTACHO,
         "50%", "Proteína",
         "Pollo, pescado, huevos, carne magra"),
        (LILA if es_carbos else RGBColor(0x8E, 0xB4, 0x7A),
         "25%",
         "Carbohidratos" if es_carbos else "Vegetales",
         "Post-entreno: camote, arroz integral" if es_carbos else "Hojas verdes, cruciferas"),
        (RGBColor(0xC9, 0xB6, 0x9E),
         "25%", "Grasas",
         "Palta, aceite de oliva, frutos secos"),
    ]
    # Círculo dividido hecho con 3 rectángulos de ancho proporcional + leyenda
    # Barras verticales: proporciones 50/25/25, bien espaciadas
    bar_x     = inches(0.80)
    bar_y     = inches(3.00)   # empieza más abajo del subtítulo
    bar_h     = inches(4.50)
    bar_total_w = inches(3.40)
    props = [0.50, 0.25, 0.25]
    gap = Pt(4)  # separación entre barras
    acum = 0
    for j, ((color, pct, label, desc), prop) in enumerate(zip(segmentos, props)):
        seg_w = int(bar_total_w * prop) - int(gap) if j < 2 else int(bar_total_w * prop)
        add_rect(s, bar_x + int(acum), bar_y, seg_w, bar_h, fill_color=color)
        # Etiqueta centrada vertical en la barra
        mid_y = bar_y + bar_h // 2
        add_textbox(s, pct,
                    bar_x + int(acum), mid_y - Pt(26),
                    seg_w, Pt(28),
                    font_name=SERIF, font_size=20, bold=True,
                    color=BLANCO, align=PP_ALIGN.CENTER, word_wrap=False)
        add_textbox(s, label,
                    bar_x + int(acum), mid_y + Pt(4),
                    seg_w, Pt(22),
                    font_name=SERIF, font_size=10, italic=True,
                    color=BLANCO, align=PP_ALIGN.CENTER, word_wrap=True)
        acum += seg_w + int(gap)

    # Leyenda a la derecha
    ley_x = bar_x + bar_total_w + inches(0.45)
    ley_w = W - ley_x - inches(0.50)
    for i, ((color, pct, label, desc), prop) in enumerate(zip(segmentos, props)):
        ly = bar_y + i * inches(1.52)
        add_rect(s, ley_x, ly + Pt(6), Pt(7), inches(1.15), fill_color=color)
        add_textbox(s, f"{pct}  {label}",
                    ley_x + Pt(16), ly + Pt(2),
                    ley_w, Pt(22),
                    font_name=SERIF, font_size=13.5, bold=True, color=OSCURO)
        add_textbox(s, desc,
                    ley_x + Pt(16), ly + Pt(26),
                    ley_w, Pt(36),
    ...
    for idx, dia in enumerate(DIAS):
        ...
        MENU_STEP = inches(2.98)   # espacio entre comidas
        IMG_W_M   = inches(1.60)
        IMG_H_M   = inches(1.00)

        for ci, (emoji, label, plato, bg, acento) in enumerate(comidas):
            y_base = inches(1.62) + ci * MENU_STEP

            # Badge de etiqueta
            add_rect(s, inches(0.50), y_base, inches(2.40), Pt(20), fill_color=bg)
            add_textbox(s, label, inches(0.60), y_base + Pt(3),
                        inches(2.30), Pt(16), font_size=8.5, color=acento, bold=True,
                        font_name=SANS)

            # Imagen rectangular a la derecha
            img_x = W - IMG_W_M - inches(0.45)
            add_image_safe(s, plato, img_x, y_base + Pt(26), IMG_W_M, IMG_H_M)

            # Nombre plato
            add_textbox(s, plato["nombre"],
                        inches(0.50), y_base + Pt(28),
                        W - inches(1.10) - IMG_W_M - inches(0.20), Pt(24),
                        font_name=SERIF, font_size=16, bold=True, color=OSCURO)

            # Descripción
            add_textbox(s, plato["desc"],
                        inches(0.50), y_base + Pt(53),
                        W - inches(1.10) - IMG_W_M - inches(0.20), Pt(30),
                        font_name=SANS, font_size=10, color=GRIS, word_wrap=True)

            # Macros
            add_textbox(s, f"{plato['prot']}g proteína  ·  {plato['kcal']} kcal",
                        inches(0.50), y_base + Pt(85),
                        inches(4.0), Pt(15),
                        font_name=SERIF, font_size=9, color=acento, italic=True)

            if ci < 2:
                add_line(s, inches(0.50), y_base + MENU_STEP - Pt(6), W - inches(1.0))
    ...
    # URLs de imágenes de ejercicio desde Unsplash (libres de uso)
    EJERCICIO_IMGS = [
        "https://images.unsplash.com/photo-1574680178050-55c6a6a96e0a?w=600&q=80",  # sentadillas
        "https://images.unsplash.com/photo-1616803689943-5601631c7fec?w=600&q=80",  # puente gluteos
        "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=600&q=80",  # flexiones
        "https://images.unsplash.com/photo-1598632640487-6ea4a4e8b963?w=600&q=80",  # remo
        "https://images.unsplash.com/photo-1434608519344-49d77a124f9a?w=600&q=80",  # zancadas
        "https://images.unsplash.com/photo-1566241440091-ec10de8db2e1?w=600&q=80",  # plancha
    ]

    ejercicios = [
        ("01", "Sentadillas",            "3 series  ×  12-15 rep", "Piernas · Glúteos"),
        ("02", "Puente de glúteos",      "3 series  ×  15-20 rep", "Glúteos · Femorales"),
        ("03", "Flexiones (rodillas)",   "3 series  ×  10-12 rep", "Pecho · Tríceps"),
    ...
    CARD_H = inches(3.50)
    IMG_EJ_H = inches(1.60)   # imagen ocupa la parte superior de la card

    for i, (num, nombre_e, series, foco_e) in enumerate(ejercicios):
        col = i % 3
        row = i // 3
        x = inches(0.50) + col * (CARD_W + inches(0.10))
        y = inches(1.65) + row * (CARD_H + inches(0.10))

        # Card con fondo ligeramente pistacho suave
        add_rect(s, x, y, CARD_W, CARD_H,
                 fill_color=BLANCO, line_color=GRIS_SOFT, line_width=Pt(0.5))

        # Intentar imagen desde repo; si no, usar bloque de color con ícono tipográfico
        ej_img_names = [f"ejercicio_{i+1:02d}", f"ej{i+1:02d}", f"ejercicio{i+1}",
                        nombre_e.lower().replace(" ","_").replace("(","").replace(")","")]
        ej_img_path = None
        for ej_name in ej_img_names:
            ej_img_path = resolve_image_path(ej_name)
            if ej_img_path:
                break
        if ej_img_path:
            try:
                add_picture_cover(s, ej_img_path, x, y, CARD_W, IMG_EJ_H)
            except Exception:
                ej_img_path = None
        if not ej_img_path:
            # Bloque de color alternado: más visual que gris plano
            colores_ej = [PISTACHO, LILA, RGBColor(0x8E,0xA8,0x7A),
                          RGBColor(0x5E,0x78,0x8A), RGBColor(0x9E,0x7A,0x5E), OSCURO]
            add_rect(s, x, y, CARD_W, IMG_EJ_H, fill_color=colores_ej[i % len(colores_ej)])

        # Franja de color sobre la imagen con el número
        add_rect(s, x, y, Pt(32), Pt(28), fill_color=PISTACHO)
        add_textbox(s, num, x + Pt(2), y + Pt(4), Pt(28), Pt(20),
                    font_name=SERIF, font_size=13, bold=True, color=BLANCO,
                    align=PP_ALIGN.CENTER, word_wrap=False)

        # Nombre del ejercicio
        add_textbox(s, nombre_e,
                    x + Pt(8), y + IMG_EJ_H + Pt(6),
                    CARD_W - Pt(16), Pt(24),
                    font_name=SERIF, font_size=12.5, bold=True, color=OSCURO, word_wrap=True)

        # Badge series
        add_rect(s, x + Pt(8), y + IMG_EJ_H + Pt(36),
                 CARD_W - Pt(16), Pt(20), fill_color=PIST_SOFT)
        add_textbox(s, series,
                    x + Pt(10), y + IMG_EJ_H + Pt(39),
                    CARD_W - Pt(20), Pt(16),
                    font_name=SANS, font_size=9, color=PISTACHO, bold=True)

        # Foco muscular
        add_textbox(s, foco_e,
                    x + Pt(8), y + CARD_H - Pt(22),
                    CARD_W - Pt(16), Pt(16),
                    font_name=SERIF, font_size=9, italic=True, color=GRIS)
