...
def generar_pptx_completo(datos, output_path):
    ...
    add_rect(s, 0, 0, W, inches(4.6), fill_color=OSCURO)

    # Línea accent pistacho a la izquierda del header
    add_rect(s, 0, 0, Pt(6), inches(4.6), fill_color=PISTACHO)
    ...
    for i, (num, nombre_e, series, musculo, acento) in enumerate(ejercicios):
        ...
        if row == 0:
            # Fila superior: sobre fondo oscuro, card semitransparente
            y = inches(2.95)
            CARD_H = inches(1.55)
            card_fill  = RGBColor(0x48, 0x52, 0x46)   # verde oscuro
            card_line  = PISTACHO
            txt_color  = BLANCO
            sub_color  = RGBColor(0xA8, 0xC0, 0xA0)
        else:
            # Fila inferior: sobre fondo vainilla
            y = inches(4.72)
            CARD_H = inches(5.98)
            card_fill  = BLANCO
            card_line  = GRIS_SOFT
        ...
        # Series — pill badge
        badge_y = y + Pt(60)
        add_rect(s, x + Pt(8), badge_y, CARD_W - Pt(16), Pt(19),
                 fill_color=acento)
        add_textbox(s, series, x + Pt(10), badge_y + Pt(3),
                    CARD_W - Pt(20), Pt(14),
                    font_name=SANS, font_size=9.5, bold=True,
                    color=BLANCO, word_wrap=False)

        if row == 1:
            # En la fila inferior hay espacio: agregar descripción del músculo + tip
            add_textbox(s, musculo, x + Pt(8), y + Pt(88),
                        CARD_W - Pt(16), Pt(16),
                        font_name=SERIF, font_size=9.5, italic=True,
                        color=sub_color, word_wrap=False)

            # Línea separadora y descripción del ejercicio
            tips_ej = {
                "Remo con botellas":   "Inclínate 45°, espalda recta. Jala los codos hacia atrás.",
                "Zancadas alternas":   "Rodilla trasera cerca del piso. Pecho erguido en todo momento.",
                "Plancha abdominal":   "Cuerpo en línea recta. Aprieta el abdomen y respira.",
            }
            tip = tips_ej.get(nombre_e, "")
            if tip:
                add_line(s, x + Pt(8), y + Pt(110), CARD_W - Pt(16))
                add_textbox(s, tip, x + Pt(8), y + Pt(118),
                            CARD_W - Pt(16), Pt(40),
