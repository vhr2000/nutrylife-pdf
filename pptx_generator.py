...
def section_header(slide, numero, titulo, subtitulo=None):
    ...
    if subtitulo:
        add_textbox(slide, subtitulo, inches(0.65), inches(2.10),
                    W - inches(1.30), Pt(16), font_size=9.5, italic=True,
...
def find_image(*names):
    """Busca imágenes habituales usando la misma lógica robusta de resolve_image_path()."""
    for name in names:
...
def add_pie_plate(slide, left, top, size, version="con_carbos"):
    """Dibuja un plato tipo torta sin colores, con porcentajes y macro dentro."""
    import matplotlib.pyplot as plt
    import tempfile
    labels = ["50%\nProteína", "25%\nCarbohidratos" if version == "con_carbos" else "25%\nVegetales", "25%\nGrasas"]
    fig, ax = plt.subplots(figsize=(4,4), dpi=220)
    ax.pie([50,25,25], labels=labels, startangle=90, counterclock=False,
           colors=["#f3f0ea", "#f3f0ea", "#f3f0ea"],
           labeldistance=0.52, textprops={"fontsize":11, "color":"#343434", "ha":"center", "va":"center"},
           wedgeprops={"linewidth":1.3, "edgecolor":"#7a7a7a"})
    ax.set(aspect="equal")
...
def generar_pptx_completo(datos, output_path):
    ...
    add_textbox(s, spaced("NUTRICIÓN AVANZADA"), inches(0.72), inches(1.06),
                inches(3.3), Pt(14), font_name=SANS, font_size=6.4, color=GRIS)
    add_textbox(s, spaced("PLAN PERSONALIZADO"), W - inches(3.35), inches(1.06),
                inches(2.7), Pt(14), font_name=SANS, font_size=6.4, color=GRIS, align=PP_ALIGN.RIGHT)
    ...
    if img_portada:
        try:
            s.shapes.add_picture(img_portada, x_img, y_img, w_img, h_img)
        except Exception:
    ...
    for i, (lbl, val, al) in enumerate(labels):
        ...
        add_textbox(s, spaced(lbl), x, inches(8.30), col_w, Pt(12),
                    font_name=SANS, font_size=5.8, color=GRIS, align=al)
        add_textbox(s, val, x, inches(8.46), col_w, Pt(28),
    ...
    add_textbox(s, "MYRIAM MÁRQUEZ GARCÍA — N U T R I C I O N I S T A",
                inches(0.65), inches(10.18), W - inches(1.30), Pt(14),
    ...
    add_textbox(s, "01", inches(0.82), inches(1.23), inches(0.42), Pt(20),
                font_name=SERIF, font_size=14, color=PISTACHO)
    add_textbox(s, "Índice", inches(1.38), inches(1.23), inches(2.2), Pt(26),
                font_name=SERIF, font_size=19, italic=True, color=OSCURO)
    ...
    for i, (num, titulo) in enumerate(secciones):
        y = y0 + i * step
        add_textbox(s, num, x_num, y, inches(0.42), Pt(13),
                    font_name=SERIF, font_size=10.0, color=PISTACHO)
        add_textbox(s, titulo, x_txt, y + Pt(1), W - inches(2.25), Pt(13),
                    font_name=SANS, font_size=9.1, color=OSCURO)
    ...
    if foto_m:
        try:
            s.shapes.add_picture(foto_m, fx, fy, fw, fh)
        except Exception:
    ...
    add_textbox(s, "Myriam Márquez García", fx - Pt(8), fy + fh + Pt(18), fw + Pt(16), Pt(18),
                font_name=SERIF, font_size=11, color=OSCURO, align=PP_ALIGN.CENTER)
    ...
    for i, par in enumerate(parrafos):
        add_textbox(s, par, inches(0.65), inches(4.70) + i * inches(1.05),
                    W - inches(1.30), Pt(48),
                    font_name=SANS, font_size=11.8, color=OSCURO, word_wrap=True)

    add_textbox(s, "Con cariño,", inches(0.65), inches(8.58), inches(3), Pt(20),
                font_size=13, italic=True, font_name=SERIF, color=OSCURO)
    add_textbox(s, "Myriam", inches(0.65), inches(8.84), inches(3), Pt(38),
                font_size=28, italic=True, font_name=SERIF, color=PISTACHO)
    ...
    for i, (num, titulo, texto) in enumerate(filosofia):
        ...
        add_textbox(s, texto, inches(1.42), y + Pt(36), W - inches(2.05), Pt(62),
                    font_size=11.0, color=OSCURO, word_wrap=True)
        if i < 2:
            add_line(s, inches(0.65), y + inches(1.42), W - inches(1.30))
    ...
    add_textbox(s, "Si hay días que solo puedes hacer dos comidas,\nestá bien — escucha tu hambre y respétala.",
                inches(0.65), inches(2.05), W - inches(1.30), Pt(52),
                font_name=SERIF, font_size=16, italic=True, color=OSCURO, align=PP_ALIGN.CENTER)
    ...
    for i, (hora, comida, desc) in enumerate(horarios):
        ...
        if i < 2:
            add_line(s, inches(0.80), y + inches(0.80), W - inches(1.60))
