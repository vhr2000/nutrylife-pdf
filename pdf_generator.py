...
def generar_pdf_completo(datos: dict, output_path: str):
    ...
    marca_data = [[
        [
            Paragraph("Myriam Márquez",
                      ParagraphStyle("marca1", fontName=FONT_SERIF_ITAL, fontSize=24,
                                     textColor=TEXTO, leading=26, spaceAfter=3)),
            Paragraph("NUTRICIONISTA · LOW CARB",
                      ParagraphStyle("marca2", fontName=FONT_SANS, fontSize=9,
    ...
    for i, dia_data in enumerate(menu_calculado):
        ...
        # Paginación:
        # i=0 Lunes → salto solo (página para él)
        # i=1 Martes, i=2 Miércoles → salto después de Miércoles
        # i=3 Jueves, i=4 Viernes → salto después de Viernes
        # i=5 Sábado, i=6 Domingo → fin
        if i == 0:
            # Lunes solo — salto de página
            story.append(PageBreak())
