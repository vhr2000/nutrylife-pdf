...
def generar_pdf_completo(datos: dict, output_path: str):
    ...
    # ----------------------------------------------------------
    from menu_engine import calcular_menu_completo, ALIMENTOS

    menu_calculado = calcular_menu_completo(datos)
    ...
    for dia_data in menu_calculado:
        # Header del día
        story.append(Paragraph(
            dia_data["dia"].upper(),
            ParagraphStyle("dia_eyebrow", fontName=FONT_SANS_BOLD, fontSize=9,
                          textColor=LILA_DARK, spaceAfter=4, letterSpacing=2)
        ))
        story.append(HRFlowable(width="100%", thickness=0.8, color=LILA,
                               spaceBefore=0, spaceAfter=10))

        for comida in dia_data["comidas"]:
            m = comida["macros"]

            comida_data = [[
                [
                    Paragraph(comida["tipo"].upper(),
                              ParagraphStyle("ct_label", fontName=FONT_SANS, fontSize=8,
                                             textColor=PISTACHO_DARK, letterSpacing=1.2,
                                             spaceAfter=2)),
                    Paragraph(comida["nombre"],
                              ParagraphStyle("ct_nombre", fontName=FONT_SERIF_ITAL, fontSize=13,
                                             textColor=TEXTO, leading=16, spaceAfter=3)),
                    Paragraph(comida["descripcion"],
                              ParagraphStyle("ct_desc", fontName=FONT_SANS, fontSize=9,
                                             textColor=TEXTO_SOFT, leading=12)),
                ],
                Paragraph(
                    f'<font name="{FONT_SANS_BOLD}" size="9" color="#6B8E5A">P</font> '
                    f'<font name="{FONT_SANS_BOLD}" size="11">{m["prot"]}g</font><br/>'
                    f'<font name="{FONT_SANS}" size="8" color="#9A9A9A">G {m["grasa"]}g · C {m["carbo"]}g</font>',
                    ParagraphStyle("ct_macros", fontName=FONT_SANS, fontSize=10,
                                  textColor=TEXTO, alignment=TA_CENTER, leading=14)
                )
            ]]
            ct = Table(comida_data, colWidths=[13.5*cm, 3.5*cm])
            ct.setStyle(TableStyle([
        ...
        t = dia_data["total"]
        total_data = [[
            Paragraph("TOTAL DEL DÍA",
                      ParagraphStyle("total_lbl", fontName=FONT_SANS_BOLD, fontSize=8,
                                     textColor=TEXTO_SOFT, letterSpacing=1.5)),
            Paragraph(
                f'<b>P {t["prot"]}g</b> · C {t["carbo"]}g · G {t["grasa"]}g · '
                f'<b>{t["kcal"]} kcal</b>',
        ...
        # Salto de página cada 2 días para no amontonar
        if dia_data["dia"] in ["Martes", "Jueves", "Sábado"]:
            story.append(PageBreak())
            if dia_data["dia"] != "Sábado":
    ...
    from menu_engine import ALIMENTOS as ALIMENTOS_DB
    categorias = {}
    for key, alimento in ALIMENTOS_DB.items():
        cat = alimento.get("categoria", "Otros")
        if cat not in categorias:
            categorias[cat] = []
        categorias[cat].append(alimento)

    for categoria, items in categorias.items():
        # Header de categoría
        story.append(Paragraph(
            categoria.upper(),
            ParagraphStyle("tn_cat", fontName=FONT_SANS_BOLD, fontSize=8,
                          textColor=PISTACHO_DARK, letterSpacing=1.5,
                          spaceAfter=6)
        ))

        # Header de columnas
        header_row = [
            Paragraph("ALIMENTO", ParagraphStyle("th", fontName=FONT_SANS_BOLD, fontSize=8,
                      textColor=TEXTO_SOFT, letterSpacing=1)),
            Paragraph("PORCIÓN", ParagraphStyle("th2", fontName=FONT_SANS_BOLD, fontSize=8,
                      textColor=TEXTO_SOFT, letterSpacing=1, alignment=TA_CENTER)),
            Paragraph("KCAL", ParagraphStyle("th3", fontName=FONT_SANS_BOLD, fontSize=8,
                      textColor=TEXTO_SOFT, letterSpacing=1, alignment=TA_CENTER)),
            Paragraph("PROT", ParagraphStyle("th4", fontName=FONT_SANS_BOLD, fontSize=8,
                      textColor=PISTACHO_DARK, letterSpacing=1, alignment=TA_CENTER)),
            Paragraph("GRASA", ParagraphStyle("th5", fontName=FONT_SANS_BOLD, fontSize=8,
                      textColor=TEXTO_SOFT, letterSpacing=1, alignment=TA_CENTER)),
            Paragraph("CARBO", ParagraphStyle("th6", fontName=FONT_SANS_BOLD, fontSize=8,
                      textColor=TEXTO_SOFT, letterSpacing=1, alignment=TA_CENTER)),
        ]
        ...
        for alimento in items:
            porcion = "100g" if alimento["unidad"] == "100g" else "1 unidad"
            fila = [
                Paragraph(alimento["nombre"],
                          ParagraphStyle("td1", fontName=FONT_SANS, fontSize=9,
                                        textColor=TEXTO)),
                Paragraph(porcion,
                          ParagraphStyle("td2", fontName=FONT_SANS, fontSize=9,
                                        textColor=TEXTO_SOFT, alignment=TA_CENTER)),
                Paragraph(str(alimento["kcal"]),
                          ParagraphStyle("td3", fontName=FONT_SANS, fontSize=9,
                                        textColor=TEXTO, alignment=TA_CENTER)),
                Paragraph(f'{alimento["prot"]}g',
                          ParagraphStyle("td4", fontName=FONT_SANS_BOLD, fontSize=9,
                                        textColor=PISTACHO_DARK, alignment=TA_CENTER)),
                Paragraph(f'{alimento["grasa"]}g',
                          ParagraphStyle("td5", fontName=FONT_SANS, fontSize=9,
                                        textColor=TEXTO_SOFT, alignment=TA_CENTER)),
                Paragraph(f'{alimento["carbo"]}g',
                          ParagraphStyle("td6", fontName=FONT_SANS, fontSize=9,
                                        textColor=TEXTO_SOFT, alignment=TA_CENTER)),
            ]
            filas.append(fila)

        t = Table(filas, colWidths=[6*cm, 2.5*cm, 2*cm, 2*cm, 2*cm, 2.5*cm])
        estilo = [
            ("VALIGN", (0,0), (-1,-1), "MIDDLE"),
