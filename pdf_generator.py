...
def draw_cover_bg(c, doc):
    ...
    # Bloque pistacho arriba derecha
    ...
    # Ícono decorativo — hoja estilizada dentro del círculo
    # Dibujada con paths simples, estilo minimalista editorial
    c.saveState()
    c.setStrokeColor(HexColor("#4A6B3A"))
    c.setFillColor(HexColor("#4A6B3A"))
    c.setLineWidth(1.2)

    # Tallo central
    c.setLineCap(1)
    c.line(cx, cy - 2.8*cm, cx, cy + 1.6*cm)

    # Hoja principal izquierda
    p = c.beginPath()
    p.moveTo(cx, cy + 0.2*cm)
    p.curveTo(cx - 1.8*cm, cy + 1.2*cm, cx - 2.2*cm, cy + 2.4*cm, cx, cy + 2.8*cm)
    p.curveTo(cx, cy + 2.8*cm, cx + 0.1*cm, cy + 1.5*cm, cx, cy + 0.2*cm)
    c.setFillAlpha(0.85)
    c.drawPath(p, fill=1, stroke=0)

    # Hoja secundaria derecha (más pequeña)
    p2 = c.beginPath()
    p2.moveTo(cx, cy - 0.6*cm)
    p2.curveTo(cx + 1.4*cm, cy + 0.2*cm, cx + 1.6*cm, cy + 1.2*cm, cx, cy + 1.2*cm)
    p2.curveTo(cx, cy + 1.2*cm, cx + 0.1*cm, cy + 0.3*cm, cx, cy - 0.6*cm)
    c.setFillAlpha(0.65)
    c.drawPath(p2, fill=1, stroke=0)

    # Hoja pequeña izquierda abajo
    p3 = c.beginPath()
    p3.moveTo(cx, cy - 1.4*cm)
    p3.curveTo(cx - 1.2*cm, cy - 0.8*cm, cx - 1.4*cm, cy - 0.0*cm, cx, cy - 0.0*cm)
    p3.curveTo(cx, cy - 0.0*cm, cx + 0.1*cm, cy - 0.8*cm, cx, cy - 1.4*cm)
    c.setFillAlpha(0.50)
...
def generar_pdf_completo(datos: dict, output_path: str):
    ...
    # Marca Myriam — logo tipográfico más grande
    marca_data = [[
        [
            Paragraph("Myriam Márquez",
                      ParagraphStyle("marca1", fontName=FONT_SERIF_ITAL, fontSize=29,
                                     textColor=TEXTO, leading=32, spaceAfter=3)),
            Paragraph("NUTRICIONISTA · LOW CARB",
                      ParagraphStyle("marca2", fontName=FONT_SANS, fontSize=11,
                                     textColor=PISTACHO_DARK, letterSpacing=2.5)),
        ],
        Paragraph("", ParagraphStyle("marca3", fontName=FONT_SANS, fontSize=9))
    ]]
    ...
    dt = Table(datos_data, colWidths=[7*cm, 10*cm])
        ...
        [
            ...
            Paragraph("kcal / día",
                      ParagraphStyle("e1u", fontName=FONT_SERIF_ITAL, fontSize=8,
                                     textColor=TEXTO_SOFT, alignment=TA_CENTER, spaceBefore=2)),
        ],
        [
            Paragraph("VERSIÓN",
    ...
    # Clase para imágenes circulares
    ...
    class CircularImage(Flowable):
        ...
        def draw(self):
            ...
            c.drawImage(self.path, 0, 0,
                       width=self.size, height=self.size,
                       preserveAspectRatio=True, anchor='c')
            c.restoreState()

    def render_comida(label, comida_data, es_almuerzo_cena=False):
        """Renderiza una comida del menú con ingredientes en líneas separadas y foto circular."""
        nombre = comida_data["nombre"]

        # Construir lista de ingredientes — uno por línea
        if es_almuerzo_cena:
            gramaje = comida_data["gramaje"]
            proteina = comida_data["proteina_nombre"]
            ingredientes_fijos = comida_data["ingredientes_fijos"]
            # Primera línea: gramaje de proteína
            ingredientes_raw = f"{gramaje}g de {proteina}, {ingredientes_fijos}"
        else:
            ingredientes_raw = comida_data["ingredientes"]
        ...
        texto_elements = [
            Paragraph(label,
                      ParagraphStyle("ct_lbl", fontName=FONT_SANS_BOLD, fontSize=10,
                                     textColor=PISTACHO_DARK, letterSpacing=1.2, spaceAfter=2)),
            Paragraph(nombre,
                      ParagraphStyle("ct_nom", fontName=FONT_SERIF_ITAL, fontSize=17,
                                     textColor=TEXTO, leading=21, spaceAfter=5)),
        ]

        # Cada ingrediente en su línea
        for ing in ingredientes_lista:
            # Capitalizar primera letra
            ing_fmt = ing[0].upper() + ing[1:] if ing else ing
            texto_elements.append(
                Paragraph(
                    f'<font color="#A8C49A">—</font>  {ing_fmt}',
                    ParagraphStyle("ct_ing", fontName=FONT_SANS, fontSize=11,
                                   textColor=TEXTO_SOFT, leading=16, spaceAfter=0)
                )
            )

        if imagen_path:
            try:
                img = CircularImage(imagen_path, size=4.0*cm)
                fila = [[texto_elements, img]]
                ct = Table(fila, colWidths=[12.5*cm, 4.5*cm])
                ct.setStyle(TableStyle([
                    ("VALIGN", (0,0), (0,0), "TOP"),
    ...
    for i, dia_data in enumerate(menu_calculado):
        ...
        if i < 6:
            ...
            # Después de Miércoles y Viernes — salto
            story.append(PageBreak())
            if i == 2:
                story.extend(chapter_header(4, "Tu menú semanal", "(continuación)"))
